...
def _parse_coord(s: str) -> float:
    ...
    """
    s = s.strip()
    # Cardinal format: digits (comma or dot decimal) + ° + N/S/E/W
    m = re.match(r'^([+-]?\d+[.,]?\d*)\s*°\s*([NSEWnsew])$', s)
    if m:
        val  = float(m.group(1).replace(',', '.'))
        card = m.group(2).upper()
        return -val if card in ('S', 'W') else val
    # Plain decimal (allow comma as decimal separator)
    try:
...
# ---------------------------------------------------------------------------
# Météo-France station catalogue helpers  (require mf_stations.csv)
# ---------------------------------------------------------------------------

MF_STATIONS_CSV = os.path.join("weather_stations_infos", "mf_stations.csv")


def _load_mf_stations() -> pd.DataFrame:
    """
...
def _display_mf_stations(df: pd.DataFrame) -> None:
...
def _select_mf_by_name(df: pd.DataFrame) -> str:
...
def _run_meteofrance():
    ...
    if sel_mode == '1':
        ...
        df_mf = _load_mf_stations()
        selected_station = _select_mf_by_name(df_mf)

    else:  # sel_mode == '4'
        df_mf = _load_mf_stations()
        selected_station = _select_mf_nearest(df_mf)
...
def _run_isd():
    ...
    selected_station = isd.select_station(stations)
